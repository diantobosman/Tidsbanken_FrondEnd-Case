...
export class NewUserDialogComponent {
  ...
  public async registerSubmit(loginForm: NgForm): Promise<void> {
    ...
    (await this.registerService.registerKeyCloak(username, password))
      .subscribe({
        next: async () => {
          ...
          this.getUserAndRegister(firstName, lastName, email, username)
        },
        error: (error) => {
    ...
    public async getUserAndRegister(firstName: string, lastName: string, email: string, username: string) {
